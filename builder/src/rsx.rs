        ...
        match tag {
            Tag::Paragraph => {
                self.start_node(parse_quote! {
                    p { class: "text-sm", class: if dark_mode().0 { "text-neutral-400" } else { "text-gray-600" } }
                });
                self.write_text();
            }
                ...
                if lang.as_deref() == Some("inject-dioxus") {
                    ...
                    static THEME: once_cell::sync::Lazy<syntect::highlighting::Theme> =
                        once_cell::sync::Lazy::new(|| {
                            let raw = include_str!("../themes/MonokaiLight.tmTheme").to_string();
                    ...
                    static DARK_THEME: once_cell::sync::Lazy<syntect::highlighting::Theme> =
                        once_cell::sync::Lazy::new(|| {
                            let raw = include_str!("../themes/Monokai.tmTheme").to_string();
                            let mut reader = std::io::Cursor::new(raw.clone());
                            ThemeSet::load_from_reader(&mut reader).unwrap()
                        });
                    ...
                    self.start_node(parse_quote! {
                        CodeBlock {
                            ...
                            contents: if dark_mode().0 { #dark_html } else { #html },
                            #fname
                        }
                    });
                }
            ...
            Tag::List(first) => {
                ...
                self.start_node(parse_quote! {
                    #name { class: "list-disc ms-6 mt-3 space-y-1.5" }
                })
            }
